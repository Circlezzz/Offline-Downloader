...
class ariapathDlg(QDialog):
    ...
    def setEXEPath(self):
        path=QFileDialog.getOpenFileName(self,'Choose aria2 path','.','aria2(aria2c.exe)')
        self.exepathLineEdit.setText(path[0])
    
    def setConfPath(self):
        path=QFileDialog.getOpenFileName(self,'Choose aria2 configure file path','.','aria2 configure file(aria2.conf)')
